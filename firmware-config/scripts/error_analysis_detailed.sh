...
analyze_errors() {
    ...
    if [ ! -f "$BUILD_DIR/build.log" ]; then
        ...
        echo "" >> $ERROR_LOG
        echo "4. 环境变量:" >> $ERROR_LOG
        env | grep -E "PATH|CCACHE|BUILD" | head -20 >> $ERROR_LOG
    ...
    # 10. 编译阶段分析
    echo "" >> $ERROR_LOG
    echo "=== 编译阶段分析 ===" >> $ERROR_LOG
    echo "配置阶段:" >> $ERROR_LOG
    grep -c "Configuration" "$BUILD_DIR/build.log" >> $ERROR_LOG
    echo "编译阶段:" >> $ERROR_LOG
    grep -c "Building\|Compiling" "$BUILD_DIR/build.log" >> $ERROR_LOG
    echo "安装阶段:" >> $ERROR_LOG
    grep -c "Installing\|Installed" "$BUILD_DIR/build.log" >> $ERROR_LOG
    ...
    if [ "$COMPILE_ERRORS" -gt 0 ]; then
        echo ""
        echo "=== 最后20行错误日志 ==="
        grep -i "Error:\|error:" "$BUILD_DIR/build.log" | tail -20
    fi
}
