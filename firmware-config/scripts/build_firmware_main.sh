...
# ============ 自动修复内容 ============
# 修复时间: Sun Feb  1 04:51:13 UTC 2026
# 来源: fix.txt
#【firmware-build.yml-00】步骤0修复版
- name: "0. 生成时间戳和设备同步检查（修复版）"
  run: |
    echo "=== 步骤 0: 生成时间戳和设备同步检查（修复版）==="
    ...
    echo "📁 工作区根目录内容:"
    ls -la "$WORKSPACE" || echo "无法列出工作区目录"
    ...
    echo "📊 搜索结果:"
    echo "  方法1 (工作区路径): $([ -f "$SUPPORT_FILE_1" ] && echo "✅ 找到" || echo "❌ 未找到")"
    echo "  方法2 (相对路径): $([ -n "$SUPPORT_FILE_2" ] && [ -f "$SUPPORT_FILE_2" ] && echo "✅ 找到: $SUPPORT_FILE_2" || echo "❌ 未找到")"
    echo "  方法3 (find搜索): $([ -n "$SUPPORT_FILE_3" ] && echo "✅ 找到: $SUPPORT_FILE_3" || echo "❌ 未找到")"
    echo "  方法4 (路径列表): $([ -n "$SUPPORT_FILE_4" ] && echo "✅ 找到: $SUPPORT_FILE_4" || echo "❌ 未找到")"
    ...
    if [ -f "$SUPPORT_FILE" ]; then
      ...
      if source "$SUPPORT_FILE" 2>/dev/null; then
        ...
        if command -v get_all_devices >/dev/null 2>&1; then
          SUPPORTED_DEVICES=$(get_all_devices 2>/dev/null || echo "无法获取设备列表")
          echo "📱 support.sh 中支持的设备: $SUPPORTED_DEVICES"
          
          SELECTED_DEVICE="${{ github.event.inputs.device_name }}"
          echo "🎯 当前选择的设备: $SELECTED_DEVICE"
          ...
          if [ "$DEVICE_IN_SUPPORT" = true ]; then
            echo "✅ 设备 '$SELECTED_DEVICE' 在 support.sh 中支持"
            if command -v get_device_description >/dev/null 2>&1; then
              DEVICE_DESC=$(get_device_description "$SELECTED_DEVICE" 2>/dev/null || echo "无描述")
              echo "📝 设备描述: $DEVICE_DESC"
            fi
          else
            echo "❌ 错误: 选择的设备 '$SELECTED_DEVICE' 不在 support.sh 的支持列表中"
            echo "📋 support.sh 支持的设备:"
            for device in "${DEVICE_ARRAY[@]}"; do
              if command -v get_device_description >/dev/null 2>&1; then
          ...
          echo "⚠️ 警告: 无法从 support.sh 读取设备列表函数"
        fi
      else
        echo "❌ 错误: 无法加载设备支持脚本（可能是语法错误）"
        exit 1
      fi
    else
      echo "❌ 错误: 无法找到设备支持文件 support.sh"
      echo "💡 调试信息:"
      echo "  当前目录: $(pwd)"
      echo "  工作区: $WORKSPACE"
      echo "  目录结构 (前3级):"
