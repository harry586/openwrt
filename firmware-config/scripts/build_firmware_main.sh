...
pre_build_error_check() {
    ...
    if [ -f "target/linux/$TARGET/Makefile" ]; then
        ...
        # 检查内核版本配置
        local kernel_config=$(grep "KERNEL_PATCHVER" "target/linux/$TARGET/Makefile" | cut -d':' -f2 | xargs)
...
build_firmware() {
    ...
    if [ "$SOURCE_REPO" = "lede" ] && [ "$SELECTED_BRANCH" = "master" ]; then
        log "🔧 LEDE master分支使用内置工具链，无需预构建编译器"
    fi
        ...
        if [ "$SELECTED_BRANCH" = "openwrt-23.05" ] || [ "$SELECTED_BRANCH" = "master" ]; then
            {
                echo "LINUX_VERSION-5.15 = .19"
                echo "LINUX_KERNEL_HASH-5.15.19 = 3e6a6b6f8c3c8e8c8f8c3c8e8c8f8c3c8e8c8f8c3c8e8c8f8c3c8e8c8f8c3c8"
                echo "KERNEL_PATCHVER:=5.15"
            } >> "$kernel_version_file"
            log "✅ 创建5.15内核版本文件"
        else
            {
                echo "LINUX_VERSION-5.4 = .19"
                echo "LINUX_KERNEL_HASH-5.4.19 = 3e6a6b6f8c3c8e8c8f8c3c8e8c8f8c3c8e8c8f8c3c8e8c8f8c3c8e8c8f8c3c8"
...
#【build_firmware_main.sh-31】编译器搜索函数（已废弃，保持兼容性）
...
#【build_firmware_main.sh-32】通用编译器搜索函数（已废弃，保持兼容性）
...
#【build_firmware_main.sh-33】简单编译器搜索函数（已废弃，保持兼容性）
...
#【build_firmware_main.sh-35】环境加载函数
load_env() {
    if [ -f "$ENV_FILE" ]; then
        source "$ENV_FILE"
...
# ============ 自动修复内容 ============
# 修复时间: Tue Feb  3 11:19:17 UTC 2026
# 来源: fix.txt
#【build_firmware_main.sh-40】主函数
main() {
    case $1 in
        # 环境设置相关
        "setup_environment")
...
# 修复时间: Tue Feb  3 11:19:17 UTC 2026
# 来源: fix.txt
#【build_firmware_main.sh-39】清理函数
cleanup() {
    log "=== 清理构建目录 ==="
    ...
    if [ -n "$GITHUB_ACTIONS" ]; then
        # 在GitHub Actions中自动清理
        user_input="y"
    else
        read -p "⚠️  确认清理构建目录? (y/N): " user_input
    fi
    
    if [[ "$user_input" =~ ^[Yy]$ ]] || [ -n "$GITHUB_ACTIONS" ]; then
        if [ -d "$BUILD_DIR" ]; then
            log "🔧 正在清理构建目录: $BUILD_DIR"
            
            # 检查是否有重要文件需要备份
            local important_files=("$ENV_FILE" "bin/targets")
            ...
            cd $BUILD_DIR 2>/dev/null && make dirclean 2>/dev/null || true
            
            # 清理特定目录
            local dirs_to_clean=("tmp" "staging_dir" "build_dir" "logs" "dl")
    ...
    # 清理临时日志文件
    rm -f /tmp/build-logs/*.log 2>/dev/null || true
    log "✅ 临时日志文件清理完成"
}
...
# 修复时间: Tue Feb  3 11:19:17 UTC 2026
# 来源: fix.txt
#【build_firmware_main.sh-38】USB驱动完整性检查函数
check_usb_drivers_integrity() {
    load_env
    cd $BUILD_DIR || handle_error "进入构建目录失败"
    ...
    local platform_specific_drivers=()
    case "$PLATFORM" in
        "ipq40xx")
            platform_specific_drivers=("kmod-phy-qcom-dwc3" "kmod-usb-dwc3-qcom" "kmod-usb-dwc3-of-simple")
            ;;
        "ramips")
            platform_specific_drivers=("kmod-usb-xhci-mtk")
            ;;
        "ath79")
            platform_specific_drivers=("kmod-usb2-ath79")
            ;;
        "mediatek")
            platform_specific_drivers=("kmod-usb-xhci-mtk")
            ;;
    ...
    local enabled_drivers=0
    local disabled_drivers=0
    ...
    for driver in "${usb_drivers[@]}"; do
        ...
        if grep -q "^CONFIG_PACKAGE_${driver}=y" .config; then
            echo "✅ $driver: 已启用"
            enabled_drivers=$((enabled_drivers + 1))
        elif grep -q "^# CONFIG_PACKAGE_${driver} is not set" .config; then
            echo "❌ $driver: 已禁用"
            disabled_drivers=$((disabled_drivers + 1))
        else
            echo "⚠️ $driver: 未配置"
    ...
    # 检查平台专用驱动
    echo "=== 平台专用驱动 ($PLATFORM) ==="
    for driver in "${platform_specific_drivers[@]}"; do
        total_drivers=$((total_drivers + 1))
        if grep -q "^CONFIG_PACKAGE_${driver}=y" .config; then
            echo "✅ $driver: 已启用"
            enabled_drivers=$((enabled_drivers + 1))
    ...
    # 文件系统驱动检查
    echo "=== 文件系统驱动 ==="
    local filesystem_drivers=("kmod-fs-ext4" "kmod-fs-vfat" "kmod-fs-ntfs3" "kmod-fs-exfat")
    for driver in "${filesystem_drivers[@]}"; do
        total_drivers=$((total_drivers + 1))
        if grep -q "^CONFIG_PACKAGE_${driver}=y" .config; then
            echo "✅ $driver: 已启用"
            enabled_drivers=$((enabled_drivers + 1))
    ...
    echo ""
    echo "=== 统计 ==="
    echo "总驱动数: $total_drivers"
    echo "已启用: $enabled_drivers"
    echo "已禁用: $disabled_drivers"
    echo "未配置: $missing_drivers"
    echo "启用率: $((enabled_drivers * 100 / total_drivers))%"
    
    # 关键驱动检查
    log "🔍 关键USB驱动检查:"
    local critical_drivers=("kmod-usb-core" "kmod-usb2" "kmod-usb-storage")
    ...
    if [ $critical_missing -eq 0 ]; then
        ...
        log "⚠️ 缺少 $critical_missing 个关键USB驱动"
        log "💡 建议在配置中启用这些驱动以确保USB功能正常"
    fi
...
# 修复时间: Tue Feb  3 11:19:17 UTC 2026
# 来源: fix.txt
#【build_firmware_main.sh-37】安装TurboACC包函数
install_turboacc_packages() {
    load_env
    cd $BUILD_DIR || handle_error "进入构建目录失败"
    ...
    if [ "$CONFIG_MODE" = "normal" ] && [ "$SELECTED_BRANCH" = "openwrt-23.05" ]; then
        log "🔧 为正常模式安装TurboACC包"
        
        ./scripts/feeds update turboacc 2>&1 | tee /tmp/turboacc_update.log || handle_error "更新TurboACC feed失败"
        
        log "安装TurboACC包..."
        ./scripts/feeds install -p turboacc luci-app-turboacc 2>&1 | tee /tmp/turboacc_install.log || handle_error "安装TurboACC包失败"
        
        log "✅ TurboACC包安装完成"
    else
        log "ℹ️ 当前配置模式或版本不需要安装TurboACC包"
    fi
}
...
# 修复时间: Tue Feb  3 11:19:17 UTC 2026
# 来源: fix.txt
#【build_firmware_main.sh-36】创建构建目录函数
create_build_dir() {
    log "=== 创建构建目录 ==="
    ...
    # 显示目录信息
    local dir_info=$(ls -ld "$BUILD_DIR" 2>/dev/null | awk '{print "权限:" $1 " 所有者:" $3 ":" $4 " 大小:" $5}')
    log "📊 构建目录信息: $dir_info"
