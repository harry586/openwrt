...
apply_config() {
    ...
    log "目标包状态: $(grep "CONFIG_PACKAGE_${PACKAGE_NAME}" .config)"
...
build_ipk() {
    ...
    if [ "$clean_build" = "true" ]; then
        log "🧹 清理包构建..."
        make package/${package_name}/clean || log "⚠️ 清理包失败，继续编译"
    fi
    ...
    # 搜索所有可能的IPK文件路径
    SEARCH_PATHS=("bin/packages/*/*/${package_name}*.ipk" "bin/targets/*/*/packages/${package_name}*.ipk")
