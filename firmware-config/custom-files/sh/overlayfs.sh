...
show_current_schedule() {
    ...
    echo "  overlay-cleanup schedule full 4 30 weekly    # 每周日4:30全面优化"
...
configure_clean_task() {
    ...
    log "配置清理定时任务: hour=$hour, minute=$minute, frequency=$frequency"
    ...
    if [ ! -f "/etc/crontabs/root" ]; then
        ...
        echo "# (/tmp/crontab_root installed on $(date '+%Y-%m-%d %H:%M:%S'))" >> /etc/crontabs/root
        echo "# Copyright (C) 2001-#$(date '+%Y') OpenWrt" >> /etc/crontabs/root
        chmod 644 /etc/crontabs/root
    fi
    
    # 删除所有包含"overlay-cleanup clean"的行
    sed -i '/overlay-cleanup clean/d' /etc/crontabs/root 2>/dev/null || true
    ...
    # 追加新的定时任务
    echo "$cron_expr /usr/sbin/overlay-cleanup clean >/dev/null 2>&1" >> /etc/crontabs/root
    ...
    log "清理定时任务配置完成: $cron_expr"
    echo "✅ 已设置清理任务: $frequency $hour:$minute"
}
...
configure_full_task() {
    ...
    log "配置全面优化定时任务: hour=$hour, minute=$minute, frequency=$frequency"
    ...
    # 删除所有包含"overlay-cleanup all"的行
    sed -i '/overlay-cleanup all/d' /etc/crontabs/root 2>/dev/null || true
    ...
    # 追加新的定时任务
    echo "$cron_expr /usr/sbin/overlay-cleanup all >/dev/null 2>&1" >> /etc/crontabs/root
    ...
    log "全面优化定时任务配置完成: $cron_expr"
    echo "✅ 已设置全面优化任务: $frequency $hour:$minute"
}
...
case "$1" in
        ...
        case "$2" in
            ...
            full)
                if [ -n "$3" ] && [ -n "$4" ] && [ -n "$5" ]; then
                    ...
                    echo "频率: daily, weekly, monthly"
                    echo "示例: overlay-cleanup schedule full 4 30 weekly"
                fi
                ;;
            auto)
                configure_auto_schedule
    ...
    *)
        ...
        echo "  overlay-cleanup schedule full 4 30 weekly   # 每周日4点30分全面优化"
        echo "  overlay-cleanup schedule auto               # 设置默认任务"
        echo "========================================"
        exit 1
        ;;
...
function get_schedule()
    ...
    local result = ""
    if nixio.fs.access("/etc/crontabs/root") then
        local f = io.popen("grep overlay-cleanup /etc/crontabs/root 2>/dev/null || echo '未设置定时任务'")
        result = f:read("*a")
        f:close()
    else
        result = "未设置定时任务"
    ...
    http.write_json({success = true, schedule = result})
end
...
<div class="cbi-map">
    ...
    <div class="cbi-section" style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        ...
        <div id="current-schedule" style="margin-top: 20px; padding: 15px; background: white; border-radius: 6px; border: 1px solid #e1e8ed;">
            ...
            <button id="refresh-schedule" class="btn-sm" style="margin-top: 10px; padding: 5px 10px;">
                <i class="icon icon-refresh"></i> 刷新定时任务
            </button>
        </div>
    </div>
...
function loadOverlayStatus() {
    ...
    statusDiv.innerHTML = '<div style="text-align: center; padding: 20px;"><div class="spinner"></div><p>正在加载状态...</p></div>';
...
function loadCurrentSchedule() {
    ...
    scheduleDiv.innerHTML = '加载中...';
    
    var xhr = new XMLHttpRequest();
    xhr.open('GET', '<%=luci.dispatcher.build_url("admin/system/overlayfs-optimize/get_schedule")%>', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            try {
                var data = JSON.parse(xhr.responseText);
                if (data.success) {
                    if (data.schedule && data.schedule.trim() !== '' && data.schedule !== '未设置定时任务') {
                        scheduleDiv.innerHTML = '<span style="color: #27ae60; font-family: monospace; white-space: pre-wrap;">' + data.schedule + '</span>';
                    } else {
                        scheduleDiv.innerHTML = '<span style="color: #e74c3c;">未设置定时任务</span>';
                    }
                } else {
                    scheduleDiv.innerHTML = '<span style="color: #e74c3c;">获取定时任务失败</span>';
                }
            } catch (e) {
                scheduleDiv.innerHTML = '<span style="color: #e74c3c;">解析定时任务失败: ' + e.message + '</span>';
            }
        } else if (xhr.readyState === 4) {
            scheduleDiv.innerHTML = '<span style="color: #e74c3c;">获取定时任务失败 (HTTP ' + xhr.status + ')</span>';
        }
    };
    xhr.send();
}
...
function performAction(action) {
    ...
    if (confirm(confirmMessages[action])) {
        showStatus('正在执行 ' + actionNames[action] + '，请稍候...', 'info');
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                try {
                    var data = JSON.parse(xhr.responseText);
                    if (data.success) {
                        showStatus('✅ ' + actionNames[action] + ' 完成: ' + data.message, 'success');
                        setTimeout(loadOverlayStatus, 2000);
                    } else {
                        showStatus('❌ ' + actionNames[action] + ' 失败: ' + data.message, 'error');
                    }
                } catch (e) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try {
                        var data = JSON.parse(xhr.responseText);
                        if (data.success) {
                            showStatus('✅ 全面优化完成: ' + data.message, 'success');
                            setTimeout(function() {
                                loadOverlayStatus();
                                loadCurrentSchedule();
                            }, 3000);
                        } else {
                            showStatus('❌ 优化失败: ' + data.message, 'error');
                        }
                    } catch (e) {
...
if [ "$RUNTIME_MODE" = "true" ]; then
    ...
    # 启用OverlayFS优化服务
    if [ -f "/etc/init.d/overlayfs-optimize" ]; then
    ...
    if [ -f "/etc/fstab" ] && [ -f "/etc/fstab.overlay" ]; then
        ...
        # 检查是否已存在overlay配置
        if ! grep -q "overlay.*/overlay" /etc/fstab 2>/dev/null; then
            # 追加配置
            cat /etc/fstab.overlay >> /etc/fstab 2>/dev/null || true
            echo "fstab优化配置已追加"
        else
