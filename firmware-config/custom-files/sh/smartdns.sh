...
config smartdns
    ...
    # ==================== 缓存优化设置（仅调整此处） ====================
    option cache_size '20000'          # 缓存条目数：20000（保持）
    option cache_persist '1'           # 持久化缓存：启用（保持）
    option serve_expired '1'           # 过期缓存服务：启用（保持）
    option prefetch_domain '1'         # 域名预获取：启用（保持）
    option rr_ttl '7200'               # 最大TTL：7200秒（从3600优化）
    option rr_ttl_min '30'             # 最小TTL：30秒（从5优化）
...
config server
    ...
    option enabled '1'
EOF
    print_status "创建主配置文件完成（仅优化缓存设置）"
}
...
if [ "$RUNTIME_MODE" = "true" ]; then
    ...
    print_status "✓ 最大TTL优化: 3600 → 7200 秒"
    print_status "✓ 最小TTL优化: 5 → 30 秒"
    print_status "✓ 缓存持久化: 启用"
    print_status "✓ 过期缓存服务: 启用"
...
echo ""
print_info "SmartDNS 缓存优化配置完成！"
print_warning "注意：测速模式保持为 'ping,tcp:80,tcp:443'，未作修改"
