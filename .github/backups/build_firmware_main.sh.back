...
# ============ 自动修复内容 ============
# 修复时间: Sun Feb  1 02:13:06 UTC 2026
# 来源: fix.txt
#【build_firmware_main.sh-04】设备支持函数（修复版）
load_device_support() {
    # 修复：支持多个可能的 support.sh 路径
    # 包括 GitHub Actions 工作区路径和常见路径
    local possible_paths=(
        "$SUPPORT_DIR/support.sh"                              # 原始路径
        "$REPO_ROOT/firmware-config/support.sh"                # 相对路径
        "/home/runner/work/openwrt/openwrt/firmware-config/support.sh"  # GitHub Actions 固定路径
        "$REPO_ROOT/support.sh"                                # 根目录
        "support.sh"                                           # 当前目录
    )
    
    # 如果有 GITHUB_WORKSPACE 环境变量，也尝试
    ...
    for support_file in "${possible_paths[@]}"; do
        if [ -f "$support_file" ]; then
            ...
            if source "$support_file" 2>/dev/null; then
                ...
                log "⚠️ 加载设备支持脚本失败: $support_file"
            fi
        fi
    done
    
    # 如果没有找到，使用 find 命令搜索
    log "⚠️ 未在预设路径找到 support.sh，开始全目录搜索..."
    local found_files=$(find "$REPO_ROOT" -name "support.sh" -type f 2>/dev/null | head -5)
    
    if [ -n "$found_files" ]; then
        log "🔍 找到以下 support.sh 文件:"
